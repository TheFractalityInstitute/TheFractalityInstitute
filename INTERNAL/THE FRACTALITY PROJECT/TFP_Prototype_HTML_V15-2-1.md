...
<head>
  ...
  <title>Fractality Bubble UI v15.2.1 - The Beginning</title>
...
<body>
  <div id="overlay">
    <h3>Fractal Node Information v15.2.1</h3>
    <div id="info"></div>
  </div>
  ...
  <script>
    window.addEventListener('DOMContentLoaded', () => {
        ...
        // --- INITIAL SETUP ---
        refreshScene();
        // NEW: Show only the Fractiverse at the start
        setFocus(1); // Set the focus to The Fractiverse (ID: 1)
        nodeMeshes.forEach(mesh => mesh.visible = (mesh.userData.id === 1)); // Only show the focused node
        controls.target.set(0,0,0); // Center the view on the focused node
    });
  </script>
</body>
</html>
