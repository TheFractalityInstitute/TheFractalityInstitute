...
<head>
  ...
  <title>Fractality Bubble UI v15.0 - The Editor (Delete)</title>
  ...
  <style>
    ...
    .delete-btn { background: #dc3545; color: white; border: none; border-radius: 4px; cursor: pointer; padding: 2px 8px; }
...
<body>
  <div id="overlay">
    <h3>Fractal Node Information v15.0</h3>
    <div id="info"></div>
  </div>
  ...
  <script>
    window.addEventListener('DOMContentLoaded', () => {
        ...
        function refreshScene() {
            ...
            nodeMeshes = [];
            // Create new meshes from current data
            ...
            // Apply layout to the new meshes
        ...
        function setFocus(newFocusId) {
            // Check if the focus target still exists
            if (!fractalNodes.find(n => n.id === newFocusId)) {
                newFocusId = 1; // Default to root if focus is deleted
            }
        ...
        function populateNodeList() {
            ...
            fractalNodes.forEach(node => {
                ...
                // Add delete button, but not for the root node
                if(node.id !== 1) {
        ...
        function deleteNode(nodeId) {
            const nodeToDelete = fractalNodes.find(n => n.id === nodeId);
            if (!nodeToDelete) return;

            const parentId = fractalConnections.find(c => c.to === nodeId)?.from;

            // Find children of the node to be deleted
            const children = fractalConnections.filter(c => c.from === nodeId);

            // Re-assign children to their grandparent
            if (parentId) {
                children.forEach(childConn => {
                    childConn.from = parentId;
                });
            }

            // Remove the node and its connection from the parent
            fractalNodes = fractalNodes.filter(n => n.id !== nodeId);
            fractalConnections = fractalConnections.filter(c => c.to !== nodeId);

            // Save the new data structure
            saveData({ nodes: fractalNodes, connections: fractalConnections });

            // Rebuild the scene and UI
            refreshScene();
            populateNodeList();
        }
